...
downloadPath = r'E:\Storage\Book\Manga\Hanime\spider1\GetBook\List\Target_2.txt'
...
def main():
    ...
    for line in lines:
        ...
        thumb_file_path = BookPath + '/' + webname + '/thumb.jpg'
        thumb_picture_url = prefix_url + 'thumb.jpg'
        # download(thumb_file_path, thumb_picture_url)

        n = 1000
        prev_sizes = [None, None, None]  # Store the sizes of the last three images
        for i in range(1, n + 1):
            file_path = BookPath + '/' + webname + '/' + str(i) + '.jpg'
            picture_url = prefix_url + str(i) + '.jpg'
            if download(file_path, picture_url):
                ...
                if i >= 3:
                    ...
                    if prev_sizes[0] == prev_sizes[1] == prev_sizes[2]:
                        ...
                        for j in range(3):
                            os.remove(BookPath + '/' + webname + '/' + str(i-j) + '.jpg')
                        break
                else:
                    print(f'Skipping {file_path} due to download failure.')  # 下载失败时跳过
